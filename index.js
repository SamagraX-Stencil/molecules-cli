...
async function fetchComponentsList(owner, repo, branch = 'main') {
    ...
    const indexJsonUrl = `https://raw.githubusercontent.com/${owner}/${repo}/${branch}/components/index.json`;
...
async function copyComponent(owner, repo, componentName, destination, branch = 'main') {
    const componentFiles = await fetchFilePaths(owner, repo, `components/${componentName}`, branch);
